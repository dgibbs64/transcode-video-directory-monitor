...
find "${inputdir}" -type f | while read -r video; do
	...
	if [ "${videomime}" == "video/x-matroska" ]; then
		...
		sleep 1
		#/home/user/convert-video.sh "${video}" >> "${scriptlog}"
		/usr/local/bin/convert-video "${video}" >> "${scriptlog}"
		exitcode=${?}
		if [ "${exitcode}" != "0" ]; then
			fn_printfailnl "Unable to convert: convert-video failed to convert video"
			fn_scriptlog "Unable to convert: convert-video failed to convert video"
			fn_fail_transfer
		else
		...
		fn_scriptlog "Conversion failed: not an mkv file"
		fn_fail_transfer
	fi
done